...
void figure::explosion_cloud_action() {
    ...
    if (progress_on_tile < 48) {
        sprite_image_id = image_id_from_group(GROUP_FIGURE_EXPLOSION) + std::clamp(progress_on_tile / 2, 0, MAX_CLOUD_IMAGE_OFFSETS);
    } else {
        sprite_image_id = image_id_from_group(GROUP_FIGURE_EXPLOSION) + MAX_CLOUD_IMAGE_OFFSETS;
    }
    anim_base = 0;
}