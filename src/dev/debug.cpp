...
void debug_text_dual_left(uint8_t* str, int x, int y, int indent, int indent2, const char *text, int value1, int value2, color_t color) {
    text_draw(string_from_ascii(text), x, y, FONT_SMALL_OUTLINED, color);
...
void draw_debug_tile(pixel_coordinate pixel, map_point point) {
    ...
    switch (DB2) {
        ...
        case 3: // ROADS
            if (b_id && map_property_is_draw_tile(grid_offset)) { //&& b->tile.grid_offset() == grid_offset
                debug_text(str, x0, y + 5, 0, "", b->road_access.x(),
                           b->has_road_access ? COLOR_GREEN : COLOR_LIGHT_RED);
                debug_text(str, x0, y + 15, 0, "", b->road_access.y(),
                           b->has_road_access ? COLOR_GREEN : COLOR_LIGHT_RED);
        ...
        case 7: // PROPER GRASS LEVEL
            ...
            if (d) debug_text(str, x, y + 10, 0, "", d, COLOR_GREEN); break;
        ...
        case 9: // FLOODPLAIN SHORE ORDER
            ...
            if (d > -1) debug_text(str, x, y + 10, 0, "", d, COLOR_LIGHT_RED); break;
        ...
        case 11: // LABOR
            if (b_id && map_property_is_draw_tile(grid_offset) && (b->labor_category != -1 || building_is_floodplain_farm(b))) {
                ...
                if (building_is_farm(b->type)) {
                    debug_text(str, x1 + 40, y + 20, 40, "fert.", map_get_fertility_for_farm(b->tile.grid_offset()),
            ...
            if (b_id && map_property_is_draw_tile(grid_offset) && (b->labor_category != -1 || building_is_floodplain_farm(b))) {
                switch (b->type) {
                    ...
                    case BUILDING_TEMPLE_COMPLEX_BAST:
                        debug_text(str, x1, y + 10, 0, "", b->data.monuments.variant, COLOR_WHITE);
                        debug_text(str, x1, y + 20, 0, "", b->data.monuments.temple_complex_attachments,
                                   COLOR_LIGHT_BLUE);
                        break;
                }
            }
...
void figure::draw_debug() {
    ...
    switch (DB1) {
        ...
        case 2: // ROUTING
            ...
            debug_text(str, pixel.x + 10, pixel.y + 40, 5, ":", roam_turn_direction,
        ...
        case 3: // RESOURCE CARRY
            if (resource_id) {
                ...
                debug_text(str, pixel.x, pixel.y + 10, indent, "", resource_amount_full,
                           resource_amount_full ? COLOR_GREEN : COLOR_FONT_MEDIUM_GRAY);
                debug_text(str, pixel.x, pixel.y + 20, indent, "", collecting_item_id,
        ...
        case 4: // BUILDING DATA
            ...
            debug_text(str, pixel.x + 20, pixel.y, 8, ":", home()->get_figure_slot(this),
                       homeID() > 0 ? COLOR_WHITE : COLOR_LIGHT_RED);
            debug_text(str, pixel.x + 0, pixel.y + 10, indent, "", destinationID(),
                       destinationID() > 0 ? COLOR_WHITE : COLOR_LIGHT_RED);
            debug_text(str, pixel.x + 20, pixel.y + 10, 8, ":", destination()->get_figure_slot(this),
                       destinationID() > 0 ? COLOR_WHITE : COLOR_LIGHT_RED);
            debug_text(str, pixel.x + 0, pixel.y + 20, indent, "", immigrant_homeID(),
                       immigrant_homeID() > 0 ? COLOR_WHITE : COLOR_LIGHT_RED);
            debug_text(str, pixel.x + 20, pixel.y + 20, 8, ":", immigrant_home()->get_figure_slot(this),
                       immigrant_homeID() > 0 ? COLOR_WHITE : COLOR_LIGHT_RED);
            break;
        case 5: // FESTIVAL
            pixel.y += 30;
        ...
        case 6: // CROSS-COUNTRY MOVEMENT
            ...
            debug_text(str, pixel.x, pixel.y, indent, "", use_cross_country); pixel.y += 10;
            debug_text(str, pixel.x, pixel.y, indent, "", cc_direction, col); pixel.y += 10;

            debug_text(str, pixel.x, pixel.y, indent, "", cc_coords.x, col);
            debug_text(str, pixel.x + 40, pixel.y, indent, "", cc_coords.y, col); pixel.y += 10;

            debug_text(str, pixel.x, pixel.y, indent, "", cc_destination.x, col);
            debug_text(str, pixel.x + 40, pixel.y, indent, "", cc_destination.y, col); pixel.y += 10;

            debug_text(str, pixel.x, pixel.y, indent, "", cc_delta_xy, col); pixel.y += 10;
            debug_text(str, pixel.x, pixel.y, indent, "", cc_delta.x, col);
            debug_text(str, pixel.x + 40, pixel.y, indent, "", cc_delta.y, col); pixel.y += 10;
            break;
    }
}
...
//        draw_debug_line(str, x + 300, y + 75, 60, "direct:", screen_end2.x, col); draw_debug_line(str, x + 300 + 40, y + 75, 60, "", screen_end2.y, col);
    ...
    if (false) {
        ...
        if (god_known_status(0) != GOD_STATUS_UNKNOWN) {
            debug_text_dual_left(str, x, y + 15, c0, c1, "Osiris:",
                                 city_data.religion.gods[0].mood,
                                 city_data.religion.gods[0].target_mood);
            debug_text_dual_left(str, x + c2, y + 15, 0, c1, "",
        ...
        {
            debug_text_dual_left(str, x, y + 25, c0, c1, "Ra:",
                                 city_data.religion.gods[1].mood,
                                 city_data.religion.gods[1].target_mood);
            debug_text_dual_left(str, x + c2, y + 25, 0, c1, "",
        ...
        if (god_known_status(2) != GOD_STATUS_UNKNOWN) {
            debug_text_dual_left(str, x, y + 35, c0, c1, "Ptah:",
                                 city_data.religion.gods[2].mood,
                                 city_data.religion.gods[2].target_mood);
            debug_text_dual_left(str, x + c2, y + 35, 0, c1, "",
        ...
        if (god_known_status(3) != GOD_STATUS_UNKNOWN) {
            debug_text_dual_left(str, x, y + 45, c0, c1, "Seth:",
                                 city_data.religion.gods[3].mood,
                                 city_data.religion.gods[3].target_mood);
            debug_text_dual_left(str, x + c2, y + 45, 0, c1, "",
        ...
        if (god_known_status(4) != GOD_STATUS_UNKNOWN) {
            debug_text_dual_left(str, x, y + 55, c0, c1, "Bast:",
                                 city_data.religion.gods[4].mood,
                                 city_data.religion.gods[4].target_mood);
            debug_text_dual_left(str, x + c2, y + 55, 0, c1, "",
        ...
        debug_text(str, x, y + 95, cl, "Slightly increased trades:",
                   city_data.religion.ra_slightly_increased_trading_months_left);
        debug_text(str, x, y + 105, cl, "Slightly reduced trades:",
                   city_data.religion.ra_slightly_reduced_trading_months_left);
        debug_text(str, x, y + 115, cl, "Harshly reduced trades:",
        ...
        debug_text(str, x, y + 155, cl, "Floods will destroy farms:",
                   city_data.religion.osiris_flood_will_destroy_active);
        y += 170;
    }
