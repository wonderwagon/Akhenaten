...
static void create_textures(void) {
    ...
    if (!tex_buffer_ui) {
        tex_buffer_ui = vita2d_create_empty_texture_format(VITA_DISPLAY_WIDTH, VITA_DISPLAY_HEIGHT,
                                                           SCE_GXM_TEXTURE_FORMAT_U8U8U8U8_ARGB);
    }
    if (config_get(CONFIG_UI_ZOOM) && !tex_buffer_city) {
        tex_buffer_city = vita2d_create_empty_texture_format(VITA_DISPLAY_WIDTH * 2, VITA_DISPLAY_HEIGHT * 2,
                                                             SCE_GXM_TEXTURE_FORMAT_U8U8U8U8_ARGB);
    }
...
int platform_screen_create(const char *title, int display_scale_percentage) {
    if (!my_window) {
        my_window = SDL_CreateWindow("Dummy window used only as event listener", 0, 0, VITA_DISPLAY_WIDTH,
                                     VITA_DISPLAY_HEIGHT, 0);
    }
    if (!my_renderer) {
    ...
    tex_buffer = vita2d_create_empty_texture_format(VITA_DISPLAY_WIDTH, VITA_DISPLAY_HEIGHT,
...
void platform_screen_render(void) {
    ...
    if (!mouse->is_touch) {
        vita2d_draw_texture(current_cursor->texture, mouse->x - current_cursor->hotspot_x,
