...
static int config_change_string_basic(int key) {
    ...
    strncpy(data.config_string_values[key].original_value, data.config_string_values[key].new_value,
...
static void button_reset_defaults(int param1, int param2) {
    ...
    for (int i = 0; i < CONFIG_STRING_MAX_ENTRIES; ++i) {
        strncpy(data.config_string_values[i].new_value, config_get_default_string_value(i),
                CONFIG_STRING_VALUE_MAX - 1);
    }
    set_language(0);
    window_invalidate();
...
static void draw_foreground(void) {
    ...
    button_border_draw(language_button.x, language_button.y, language_button.width, language_button.height,
