...
static generic_button build_menu_buttons[] = {{0, 0, 160, 20, button_menu_item, button_none, 0, 0},
...
static void draw_menu_buttons(void) {
    ...
    for (int i = 0; i < data.num_items; i++) {
        ...
        lang_text_draw_centered(48,
                                MENU_TYPES[data.selected_submenu][i],
                                x_offset - 170,
                                data.y_offset + 113 + 24 * i,
                                160,
...
static void button_menu_item(int index, int param2) {
    ...
    case MENU_ELEVATION:
        switch (index) {
        case 0:
            editor_tool_set_type(TOOL_RAISE_LAND);
            break;
        case 1:
            editor_tool_set_type(TOOL_LOWER_LAND);
            break;
        case 2:
            editor_tool_set_type(TOOL_ACCESS_RAMP);
            break;
        }
        break;
    case MENU_PEOPLE_POINTS:
        switch (index) {
        case 0:
            editor_tool_set_type(TOOL_ENTRY_POINT);
            break;
        case 1:
            editor_tool_set_type(TOOL_EXIT_POINT);
            break;
        }
        break;
    case MENU_RIVER_POINTS:
        switch (index) {
        case 0:
            editor_tool_set_type(TOOL_RIVER_ENTRY_POINT);
            break;
        case 1:
            editor_tool_set_type(TOOL_RIVER_EXIT_POINT);
            break;
        }
        break;
    case MENU_NATIVE_BUILDINGS:
        switch (index) {
        case 0:
            editor_tool_set_type(TOOL_NATIVE_HUT);
            break;
        case 1:
            editor_tool_set_type(TOOL_NATIVE_CENTER);
            break;
        case 2:
            editor_tool_set_type(TOOL_NATIVE_FIELD);
            break;
        }
        break;
