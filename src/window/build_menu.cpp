    ...
    for (int i = 0; i < data.num_items; i++) {
        ...
        if (is_all_button(type))
            lang_text_draw_centered(
              52, 19, x_offset - label_margin + label_offset, data.y_offset + 113 + 24 * i, 176, font);
            ...
            lang_text_draw_centered(
              189, index, x_offset - label_margin + label_offset, data.y_offset + 113 + 24 * i, 176, font);
        } else
            lang_text_draw_centered(
              28, type, x_offset - label_margin + label_offset, data.y_offset + 113 + 24 * i, 176, font);
...
static int handle_build_submenu(const mouse* m) {
    return generic_buttons_handle_mouse(
      m, get_sidebar_x_offset() - 258, data.y_offset + 110, build_menu_buttons, data.num_items, &data.focus_button_id);
}
...
void window_build_menu_show(int submenu) {
    if (init(submenu)) {
        window_type window = {WINDOW_BUILD_MENU, draw_background, draw_foreground, handle_input, 0};
        window_show(&window);
    }
}
