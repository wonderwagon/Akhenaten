...
void window_advisors_draw_dialog_background(void) {
    imagedrawnamespace::image_draw_fullscreen_background(image_id_from_group(GROUP_ADVISOR_BACKGROUND));
    graphics_in_dialog();
    if (GAME_ENV == ENGINE_ENV_C3)
        imagedrawnamespace::image_draw(image_id_from_group(GROUP_PANEL_WINDOWS) + 13, 0, 432);
    else if (GAME_ENV == ENGINE_ENV_PHARAOH)
        imagedrawnamespace::image_draw(image_id_from_group(GROUP_MENU_ADVISOR_LAYOUT), 0, 432);
    ...
    for (int i = 0; i < 14; i++) {
        ...
        if (GAME_ENV == ENGINE_ENV_C3) {
            ...
            if (current_advisor && i == (current_advisor % 13) - 1)
                selected_offset = 13;
            imagedrawnamespace::image_draw(image_id_from_group(GROUP_ADVISOR_ICONS) + i + selected_offset, 48 * i + 12, 441);
        }
//        else if (GAME_ENV == ENGINE_ENV_PHARAOH)
