...
static void draw_background(void) {
    ...
    if (data.type == INTERMEZZO_MISSION_BRIEFING) {
        if (scenario_is_custom())
            imagedrawnamespace::image_draw(image_base + 1, x_offset, y_offset);
        else {
            if (GAME_ENV == ENGINE_ENV_C3)
                imagedrawnamespace::image_draw(image_base + 1 + 2 * mission, x_offset, y_offset);
            else if (GAME_ENV == ENGINE_ENV_PHARAOH)
                imagedrawnamespace::image_draw(image_base + 1 + (mission >= 20), x_offset, y_offset);
        }
    } else if (data.type == INTERMEZZO_FIRED)
        imagedrawnamespace::image_draw(image_base, x_offset, y_offset);
    else if (data.type == INTERMEZZO_WON) {
        if (scenario_is_custom())
            imagedrawnamespace::image_draw(image_base + 2, x_offset, y_offset);
        else
            if (GAME_ENV == ENGINE_ENV_C3)
                imagedrawnamespace::image_draw(image_base + 2 + 2 * mission, x_offset, y_offset);
            else if (GAME_ENV == ENGINE_ENV_PHARAOH)
                imagedrawnamespace::image_draw(image_base, x_offset, y_offset);
    }
}
