...
void window_building_draw_roadblock_orders_foreground(building_info_context *c) {
    ...
    for (int i = 0; i < size_of_orders_permission_buttons; i++) {
        imagedrawnamespace::image_draw(image_id_from_group(ids[i * 2]) + 4, c->x_offset + 32, y_offset + 46 + 32 * i);
        imagedrawnamespace::image_draw(image_id_from_group(ids[i * 2 + 1]) + 4, c->x_offset + 64, y_offset + 46 + 32 * i);
