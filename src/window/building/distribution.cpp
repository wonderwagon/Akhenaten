...
static image_button orders_decrease_arrows[] = {
  {0,
   ...
   GROUP_TINY_ARROWS,
   3,
   order_quantity_increase_decrease,
   order_quantity_increase_decrease,
   1,
   0,
   true},
...
int window_building_handle_mouse_dock(const mouse* m, building_info_context* c) {
    return generic_buttons_handle_mouse(
      m, c->x_offset + 80, c->y_offset + 16 * c->height_blocks - 34, go_to_orders_button, 1, &data.focus_button_id);
}
int window_building_handle_mouse_dock_orders(const mouse* m, building_info_context* c) {
    ...
    if (generic_buttons_handle_mouse(
          m, c->x_offset + 180, y_offset + 46, orders_resource_buttons, 15, &data.resource_focus_button_id)) {
        return 1;
    }
    return generic_buttons_handle_mouse(
...
void window_building_draw_market(building_info_context* c) {
    ...
    if (!c->has_road_access)
        ...
        text_draw_number(
          b->data.market.inventory[INVENTORY_GOOD1], '@', " ", c->x_offset + 64, c->y_offset + Y_GOODS + 4, font);
        ...
        text_draw_number(
          b->data.market.inventory[INVENTORY_GOOD2], '@', " ", c->x_offset + 174, c->y_offset + Y_GOODS + 4, font);
        ...
        text_draw_number(
          b->data.market.inventory[INVENTORY_GOOD3], '@', " ", c->x_offset + 284, c->y_offset + Y_GOODS + 4, font);
        ...
        text_draw_number(
          b->data.market.inventory[INVENTORY_GOOD4], '@', " ", c->x_offset + 394, c->y_offset + Y_GOODS + 4, font);
    }
    inner_panel_draw(c->x_offset + 16, c->y_offset + 136, c->width_blocks - 2, 4);
    window_building_draw_employment(c, 142);
}
...
void window_building_draw_market_orders_foreground(building_info_context* c) {
    ...
    for (int i = 0; i < list->size; i++) {
        ...
        if (data.resource_focus_button_id - 1 == i)
            button_border_draw(line_x - 10,
                               y_offset + 46 + line_y,
                               orders_resource_buttons[i].width,
                               orders_resource_buttons[i].height,
                               true);

        // order status
        draw_order_instruction(
          INSTR_STORAGE_YARD, nullptr, resource, line_x, y_offset + 51 + line_y, is_good_accepted(i, b));
    }
...
int window_building_handle_mouse_granary(const mouse* m, building_info_context* c) {
    ...
    generic_buttons_handle_mouse(m,
                                 c->x_offset + 58,
                                 c->y_offset + 19 * c->height_blocks - 82,
                                 warehouse_distribution_permissions_buttons,
                                 1,
...
int window_building_handle_mouse_granary_orders(const mouse* m, building_info_context* c) {
    ...
    if (image_buttons_handle_mouse(
          m, c->x_offset + 165, y_offset + 49, orders_decrease_arrows, 20, &data.decr_arrow_focus_button_id))
        handled = true;
    if (image_buttons_handle_mouse(
          m, c->x_offset + 165 + 18, y_offset + 49, orders_increase_arrows, 20, &data.decr_arrow_focus_button_id))
        handled = true;
    if (handled)
        return 1;
    ...
    if (GAME_ENV == ENGINE_ENV_C3)
        return generic_buttons_handle_mouse(
          m, c->x_offset + 80, y_offset + 404, granary_order_buttons, 3, &data.orders_focus_button_id);
    else if (GAME_ENV == ENGINE_ENV_PHARAOH)
        return generic_buttons_handle_mouse(
          m, c->x_offset + 80, y_offset + 404, granary_order_buttons, 2, &data.orders_focus_button_id);
}
void window_building_draw_granary(building_info_context* c) {
    ...
    if (!c->has_road_access)
        ...
        lang_text_draw_amount(
          8, 16, total_stored, c->x_offset + 34 + width, c->y_offset + 40, FONT_NORMAL_BLACK_ON_LIGHT);

        width = lang_text_draw(98, 3, c->x_offset + 220, c->y_offset + 40, FONT_NORMAL_BLACK_ON_LIGHT);
        lang_text_draw_amount(8,
                              16,
                              b->data.granary.resource_stored[RESOURCE_NONE],
                              c->x_offset + 220 + width,
                              c->y_offset + 40,
                              FONT_NORMAL_BLACK_ON_LIGHT);
        ...
        if (food1) { // wheat
            ...
            width = text_draw_number(b->data.granary.resource_stored[food1],
                                     '@',
                                     " ",
                                     c->x_offset + 68,
                                     c->y_offset + 75,
        ...
        if (food3) { // vegetables
            ...
            width = text_draw_number(b->data.granary.resource_stored[food3],
                                     '@',
                                     " ",
                                     c->x_offset + 68,
                                     c->y_offset + 99,
...
void window_building_draw_warehouse(building_info_context* c) {
    ...
    if (cartpusher->state == FIGURE_STATE_ALIVE) {
        ...
        lang_text_draw_multiline(
          99, 17, c->x_offset + 64, c->y_offset + 223, 16 * (c->width_blocks - 6), FONT_NORMAL_BLACK_ON_DARK);
    } else if (b->num_workers) {
        // cartpusher is waiting for orders
        lang_text_draw_multiline(
          99, 15, c->x_offset + 32, c->y_offset + 223, 16 * (c->width_blocks - 4), FONT_NORMAL_BLACK_ON_DARK);
    }
