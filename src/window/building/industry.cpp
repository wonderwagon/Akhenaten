...
static void draw_farm(building_info_context *c, int help_id, const char *sound_file, int group_id, int resource) {
    ...
    imagedrawnamespace::image_draw(image_id_from_group(GROUP_RESOURCE_ICONS) + resource, c->x_offset + 10, c->y_offset + 10);
...
static void draw_workshop(building_info_context *c, int help_id, const char *sound_file, int group_id, int resource, int input_resource) {
    ...
    imagedrawnamespace::image_draw(image_id_from_group(GROUP_RESOURCE_ICONS) + input_resource, c->x_offset + 32, c->y_offset + 56);
...
void window_building_draw_hunting_lodge(building_info_context *c) {
    ...
    building *b = building_get(c->building_id);
    imagedrawnamespace::image_draw(image_id_from_group(GROUP_RESOURCE_ICONS) + RESOURCE_GAMEMEAT, c->x_offset + 32, c->y_offset + 56);
...
void window_building_draw_wharf(building_info_context *c) {
    ...
    imagedrawnamespace::image_draw(image_id_from_group(GROUP_RESOURCE_ICONS) + RESOURCE_MEAT_C3 +
...
//                window_building_draw_description(c, group_id, 7); // working on both floodplains and monuments (unused?)
            }
            else
