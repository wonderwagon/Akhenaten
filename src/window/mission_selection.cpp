...
static void draw_background(void) {
    ...
    imagedrawnamespace::image_draw_fullscreen_background(image_id_from_group(GROUP_SELECT_MISSION_BACKGROUND));
    graphics_in_dialog();
    imagedrawnamespace::image_draw(image_id_from_group(GROUP_SELECT_MISSION) + BACKGROUND_IMAGE_OFFSET[rank], 0, 0);
...
static void draw_foreground(void) {
    ...
    if (data.choice == 0) {
        imagedrawnamespace::image_draw(data.focus_button == 1 ? image_id + 1 : image_id, x_peaceful, y_peaceful);
        imagedrawnamespace::image_draw(data.focus_button == 2 ? image_id + 1 : image_id, x_military, y_military);
    } else if (data.choice == 1) {
        imagedrawnamespace::image_draw(data.focus_button == 1 ? image_id + 1 : image_id + 2, x_peaceful, y_peaceful);
        imagedrawnamespace::image_draw(data.focus_button == 2 ? image_id + 1 : image_id, x_military, y_military);
    } else {
        imagedrawnamespace::image_draw(data.focus_button == 1 ? image_id + 1 : image_id, x_peaceful, y_peaceful);
        imagedrawnamespace::image_draw(data.focus_button == 2 ? image_id + 1 : image_id + 2, x_military, y_military);
    }
    graphics_reset_dialog();
}
