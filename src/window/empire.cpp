...
static void draw_trade_city_info(const empire_object* object, const empire_city* city) {
    ...
    if (city->is_open) {
        ...
        for (int resource = RESOURCE_MIN; resource < RESOURCES_MAX; resource++) {
            ...
            int text_width = text_draw_number(
              trade_now, '@', "", x_offset + column_offset + 150, y_offset + INFO_Y_TRADED + row_offset, traded_font);
            text_width += lang_text_draw(
              47, 12, x_offset + column_offset + 148 + text_width, y_offset + INFO_Y_TRADED + row_offset, traded_font);
            text_draw_number(trade_max,
                             '@',
                             "",
                             x_offset + column_offset + 138 + text_width,
                             y_offset + INFO_Y_TRADED + row_offset,
                             traded_font);
            index++;
        }
        // city buys
        ...
        // trade open button
        spacing = lang_text_draw_amount(
          8, 0, city->cost_to_open, x_offset + 40, y_offset + INFO_Y_FOOTER_1, FONT_OBJECT_INFO[GAME_ENV]);
        lang_text_draw(
          47, 6 + city->is_sea_trade, x_offset + spacing + 40, y_offset + INFO_Y_FOOTER_1, FONT_OBJECT_INFO[GAME_ENV]);
