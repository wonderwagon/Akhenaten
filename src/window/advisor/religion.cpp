...
static void draw_festival_info(int y_offset) {
    ...
    imagedrawnamespace::image_draw(image_id_from_group(GROUP_PANEL_WINDOWS) + 15, 460, 255 + y_offset);
...
//        font = FONT_NORMAL_RED;
    ...
    for (int i = 0; i < city_god_wrath_bolts(god) / 10; i++)
        imagedrawnamespace::image_draw(image_id_from_group(GROUP_GOD_BOLT), 10 * i + width + 460, y_offset - 4);
    for (int i = 0; i < city_god_happy_angels(god) / 10; i++)
        imagedrawnamespace::image_draw(image_id_from_group(GROUP_GOD_ANGEL), 10 * i + width + 460, y_offset - 4);
}
...
static int draw_background(void) {
    ...
    imagedrawnamespace::image_draw(image_id_from_group(GROUP_ADVISOR_ICONS) + 9, 10, 10);
