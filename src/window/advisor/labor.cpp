...
static int draw_background(void) {
    ...
    imagedrawnamespace::image_draw(image_id_from_group(GROUP_ADVISOR_ICONS), 10, 10);
...
static void draw_foreground(void) {
    ...
    for (int i = 0; i < 9; i++) {
        ...
        if (cat->priority) {
            imagedrawnamespace::image_draw(image_id_from_group(GROUP_LABOR_PRIORITY_LOCK), 70, y_offset - 2);
