...
static int draw_background(void) {
    ...
    image_draw(image_id_from_group(GROUP_ADVISOR_ICONS) + 1, 10, 10);
    ...
    int text_x = 80;
    if (num_legions <= 0) {
        image_draw(image_id_from_group(GROUP_BULLET), bullet_x, 359);
        lang_text_draw(51, enemy_text_id, text_x, 358, FONT_NORMAL_BLACK);

        image_draw(image_id_from_group(GROUP_BULLET), bullet_x, 379);
        lang_text_draw(51, distant_battle_text_id, text_x, 378, FONT_NORMAL_BLACK);
    } else {
        // has forts
        image_draw(image_id_from_group(GROUP_BULLET), bullet_x, 349);
        ...
        image_draw(image_id_from_group(GROUP_BULLET), bullet_x, 369);
        lang_text_draw(51, enemy_text_id, text_x, 368, FONT_NORMAL_BLACK);

        image_draw(image_id_from_group(GROUP_BULLET), bullet_x, 389);
    ...
    for (int i = 0; i < 6 && i < num_legions; i++) {
        ...
        image_draw(image_id_from_group(GROUP_FIGURE_FORT_STANDARD_ICONS) + m->legion_id, 32, 82 + 44 * i);
