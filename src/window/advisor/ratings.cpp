...
//            has_reached = 0;
    ...
    imagedrawnamespace::image_draw(image_base, x_offset - 4, y);
    for (int i = 0; i < 2 * value_to_draw; i++)
        imagedrawnamespace::image_draw(image_base + 1, x_offset + 11, --y);
    if (has_reached)
        imagedrawnamespace::image_draw(image_base + 2, x_offset - 6, y - 50);
}
...
static int draw_background(void) {
    ...
    imagedrawnamespace::image_draw(image_id_from_group(GROUP_ADVISOR_ICONS) + 3, 10, 10);
    ...
    imagedrawnamespace::image_draw(image_id_from_group(GROUP_RATINGS_BACKGROUND), 60, 48 - 10);
