...
const event_ph_t *get_scenario_event(int id) {
    return &scenario.events.event_list[id];
}
static bool is_valid_event_index(int id) {
    if (id >= MAX_EVENTS || id >= *scenario.events.num_of_events)
        return false;
    return true;
}

static int get_auto_reason_phrase_id(int param_1, int param_2) {
    return scenario.events.auto_phrases[param_1][param_2];
}
uint8_t *get_eventmsg_text(int group_id, int index) {
    int eventmsg_id = scenario.events.eventmsg_group_offsets[group_id] + index;
    return &scenario.events.eventmsg_phrases_data[scenario.events.eventmsg_line_offsets[eventmsg_id]];
}
...
static void event_fire(int id, bool via_event_trigger, int action_status, int caller_event_id = -1) {
    ...
    event_ph_t *event = &scenario.events.event_list[id];
    ...
    // Pharaoh duplicates the fired events and sets trigger type to "12" for some reason. I have no idea why.
...
void scenario_event_process() {
    for (int i = 0; i < *scenario.events.num_of_events; i++) {
        event_ph_t *event = &scenario.events.event_list[i];
...
void scenario_events_load_state(buffer *buf) {
    ...
    scenario.events.num_of_events = &(scenario.events.event_list[0].num_total_header);
...
bool eventmsg_load(void) {
    ...
    for (int i = 0; i < NUM_PHRASES; i++) {
        ...
        scenario.events.eventmsg_line_offsets[i] = offset;
        offset += size;
        ...
        if (is_line_standalone_group(start_of_line, ptr - start_of_line)){
            if (group_offset < 298)
                scenario.events.eventmsg_group_offsets[group_offset] = i;
            else // Pyramid & extra messages
                scenario.events.eventmsg_group_offsets[i] = i;
//                scenario.events.eventmsg_group_offsets[group_offset + 252] = i;
            group_offset++;
        } else {
            scenario.events.eventmsg_group_offsets[group_offset_extra + 298] = i;
            group_offset_extra++;
        }
    }
    buf2.reset_offset();
    buf2.read_raw(&scenario.events.eventmsg_phrases_data, offset);
...
bool eventmsg_auto_phrases_load(void) {
    ...
    for (int i = 0; i < NUM_AUTO_PHRASE_VARIANTS; i++) {
        ...
        for (int a = 0; a < 36; a++)
            ptr = get_value(ptr, end_ptr, &scenario.events.auto_phrases[i][a]);
    }
