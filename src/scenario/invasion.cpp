...
void scenario_invasion_init(void) {
    ...
    for (int i = 0; i < MAX_INVASIONS[GAME_ENV]; i++) {
        ...
        scenario_data.invasions[i].month = 2 + (random_byte() & 7);
        if (scenario_data.invasions[i].type == INVASION_TYPE_LOCAL_UPRISING ||
            scenario_data.invasions[i].type == INVASION_TYPE_DISTANT_BATTLE) {
            continue;
        }
        for (int year = 1; year < 8; year++) {
            ...
            warning->months_to_go = 12 * scenario_data.invasions[i].year;
            warning->months_to_go += scenario_data.invasions[i].month;
...
static int start_invasion(int enemy_type, int amount, int invasion_point, int attack_type, int invasion_id) {
    ...
    if (enemy_type == ENEMY_11_CAESAR) {
        ...
        if (num_points > 0) {
            while (scenario_data.invasion_points[invasion_point].x == -1) {
                invasion_point++;
        ...
        x = scenario_data.invasion_points[invasion_point].x;
        y = scenario_data.invasion_points[invasion_point].y;
    }
    if (x == -1 || y == -1) {
    ...
    else if (y >= scenario_data.map.height - 1)
        orientation = DIR_0_TOP_RIGHT;
    else if (x == 0)
        orientation = DIR_2_BOTTOM_RIGHT;
    else if (x >= scenario_data.map.width - 1)
...
void scenario_invasion_process(void) {
    int enemy_id = scenario_data.enemy_id;
    for (int i = 0; i < MAX_INVASION_WARNINGS; i++) {
        ...
        if (game_time_year() >= scenario_data.start_year + scenario_data.invasions[warning->invasion_id].year &&
            game_time_month() >= scenario_data.invasions[warning->invasion_id].month) {
            ...
            // enemy invasions
            if (scenario_data.invasions[warning->invasion_id].type == INVASION_TYPE_ENEMY_ARMY) {
                int grid_offset = start_invasion(
                        ENEMY_ID_TO_ENEMY_TYPE[enemy_id],
                        scenario_data.invasions[warning->invasion_id].amount,
                        scenario_data.invasions[warning->invasion_id].from,
                        scenario_data.invasions[warning->invasion_id].attack_type,
                        warning->invasion_id);
                if (grid_offset > 0) {
            ...
            if (scenario_data.invasions[warning->invasion_id].type == INVASION_TYPE_CAESAR) {
                int grid_offset = start_invasion(
                        ENEMY_11_CAESAR,
                        scenario_data.invasions[warning->invasion_id].amount,
                        scenario_data.invasions[warning->invasion_id].from,
    ...
    for (int i = 0; i < MAX_INVASIONS[GAME_ENV]; i++) {
        if (scenario_data.invasions[i].type == INVASION_TYPE_LOCAL_UPRISING) {
            if (game_time_year() == scenario_data.start_year + scenario_data.invasions[i].year &&
                game_time_month() == scenario_data.invasions[i].month) {
                int grid_offset = start_invasion(
                        ENEMY_0_BARBARIAN,
                        scenario_data.invasions[i].amount,
                        scenario_data.invasions[i].from,
                        scenario_data.invasions[i].attack_type,
                        i);
                if (grid_offset > 0)
