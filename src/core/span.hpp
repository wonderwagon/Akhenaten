...
namespace std {
  ...
  public:
    constexpr span() noexcept : m_storage(nullptr), m_size(0) {}

    template <typename It>
    constexpr span(It it, size_type count) noexcept : m_storage(it), m_size(count) {}

    template <typename It>
    constexpr span(It it, It end) noexcept : m_storage(it), m_size(end - it) {}

    template <std::size_t N>
    constexpr span(element_type (&arr)[N]) noexcept : m_storage(arr), m_size(N) {}

    template <typename U, std::size_t N>
    constexpr span(std::array<U, N>& arr) noexcept :  m_storage(arr.begin()), m_size(N) {}
