...
static void pk_implode_write_copy_length_offset(struct pk_comp_buffer* buf, struct pk_copy_length_offset copy) {
    ...
    if (copy.length == 2) {
        pk_implode_write_bits(buf, pk_copy_offset_bits[copy.offset >> 2], pk_copy_offset_code[copy.offset >> 2]);
        pk_implode_write_bits(buf, 2, copy.offset & 3);
    } else {
        pk_implode_write_bits(buf,
                              pk_copy_offset_bits[copy.offset >> buf->window_size],
    ...
    while (!eof) {
        ...
        while (input_ptr < input_end) {
            ...
            if (write_copy) {
                ...
                // Write literal
                pk_implode_write_bits(buf,
                                      buf->codeword_bits[buf->input_data[input_ptr]],
...
static void zip_output_func(uint8_t* buffer, int length, struct pk_token* token) {
    ...
    if (token->output_ptr >= token->output_length) {
        logs::error("COMP2 Out of buffer space.");
        token->stop = 1;
        return;
    }
    //    // for debugging
...
int zip_compress(const void* input_buffer, int input_length, void* output_buffer, int* output_length) {
    ...
    if (pk_error || token.stop) {
        logs::error("COMP Error occurred while compressing.");
        ok = 0;
    } else {
...
int zip_decompress(const void* input_buffer, int input_length, void* output_buffer, int* output_length) {
    ...
    if (pk_error || token.stop) {
        logs::error("COMP Error uncompressing.");
        ok = 0;
    } else {
        *output_length = token.output_ptr;
        ok = *output_length;
