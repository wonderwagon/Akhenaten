    ...
    case 10:
        ...
        if (wait_ticks > 10) {
            ...
            if (move_on > 0 && figure_trade_caravan_can_sell(this, destination(), empire_city_id)) {
                ...
                move_on++;
            if (move_on == 2)
                go_to_next_warehouse(tile.x(), tile.y(), -1);
        }
        anim_frame = 0;
        break;
...
void figure::native_trader_action() {
    ...
    case FIGURE_ACTION_162_NATIVE_TRADER_CREATED:
        ...
        if (wait_ticks > 10) {
            wait_ticks = 0;
            map_point tile;
            int building_id = get_closest_warehouse(tile.x(), tile.y(), 0, -1, &tile);
            if (building_id) {
    ...
    case FIGURE_ACTION_114_TRADE_SHIP_ANCHORED:
        ...
        if (wait_ticks > 40) {
            ...
            if (dock_id) {
                ...
                //                    destination_tile.x() = tile.x();
                //                    destination_tile.y() = tile.y();
            } else if (map_figure_at(tile.grid_offset()) != id && building_dock_get_queue_destination(id, &tile)) {
                action_state = FIGURE_ACTION_113_TRADE_SHIP_GOING_TO_DOCK_QUEUE;
                destination_tile = tile;
                //                    destination_tile.x() = tile.x();
                //                    destination_tile.y() = tile.y();
