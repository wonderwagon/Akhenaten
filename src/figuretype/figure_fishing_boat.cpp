...
void figure_fishing_boat::figure_action() {
    ...
    if (action_state() != FIGURE_ACTION_190_FISHING_BOAT_CREATED && b->data.industry.fishing_boat_id != id()) {
        tile2i tile;
        int bid = map_water_get_wharf_for_new_fishing_boat(&base, &tile);
        b = building_get(bid);
        if (b->id) {
            set_home(b->id);
    ...
    case FIGURE_ACTION_190_FISHING_BOAT_CREATED:
        ...
        if (wait_ticks >= 50) {
            wait_ticks = 0;
            tile2i tile;
            int wharf_id = map_water_get_wharf_for_new_fishing_boat(&base, &tile);
            if (wharf_id) {
    ...
    if (direction() == DIR_FIGURE_NONE) {
        tile2i tile;
        int bid = map_water_find_alternative_fishing_boat_tile(&base, &tile);
        if (bid) {
            route_remove();
            destination_tile = tile;
    ...
    if (action_state() == FIGURE_ACTION_192_FISHING_BOAT_FISHING) {
        base.sprite_image_id = image_group(IMG_FISHING_BOAT_WORK) + dir + 8;
    } else {
        base.sprite_image_id = image_group(IMG_FISHING_BOAT) + dir + 8;
    }
}
