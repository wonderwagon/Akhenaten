...
static void add_fort(int type, building* fort) {
    ...
    map_building_tiles_add(
      fort->id, fort->tile.x(), fort->tile.y(), fort->size, image_id_from_group(GROUP_BUILDING_FORT), TERRAIN_BUILDING);
...
void BuildPlanner::setup_build(
  int type) { // select building for construction, set up main terrain restrictions/requirements
