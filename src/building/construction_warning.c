    ...
    if (!has_warning) {
        if (type == BUILDING_FOUNTAIN || type == BUILDING_BATHHOUSE) {
            ...
            if (map_terrain_is(grid_offset, TERRAIN_GROUNDWATER_RANGE))
                ...
                if (map_terrain_is(grid_offset + map_grid_delta(1, 0), TERRAIN_GROUNDWATER_RANGE) ||
                    map_terrain_is(grid_offset + map_grid_delta(0, 1), TERRAIN_GROUNDWATER_RANGE) ||
                    map_terrain_is(grid_offset + map_grid_delta(1, 1), TERRAIN_GROUNDWATER_RANGE)) {
                    has_water = 1;
                }
            }
            if (!has_water)
