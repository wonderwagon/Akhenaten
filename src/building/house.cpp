...
static const struct {
    ...
    int num_types;
        ...
        {GROUP_BUILDING_HOUSE_TENT,     0, 2},
        {GROUP_BUILDING_HOUSE_TENT,     2, 2},
        {GROUP_BUILDING_HOUSE_SHACK,    0, 2},
        {GROUP_BUILDING_HOUSE_SHACK,    2, 2},
        {GROUP_BUILDING_HOUSE_HOVEL,    0, 2},
        {GROUP_BUILDING_HOUSE_HOVEL,    2, 2},
        {GROUP_BUILDING_HOUSE_CASA,     0, 2},
        {GROUP_BUILDING_HOUSE_CASA,     2, 2},
        {GROUP_BUILDING_HOUSE_INSULA_1, 0, 2},
        {GROUP_BUILDING_HOUSE_INSULA_1, 2, 2},
        {GROUP_BUILDING_HOUSE_INSULA_2, 0, 2},
        {GROUP_BUILDING_HOUSE_INSULA_2, 2, 2},
        {GROUP_BUILDING_HOUSE_VILLA_1,  0, 2},
        {GROUP_BUILDING_HOUSE_VILLA_1,  2, 2},
        {GROUP_BUILDING_HOUSE_VILLA_2,  0, 1},
        {GROUP_BUILDING_HOUSE_VILLA_2,  1, 1},
        {GROUP_BUILDING_HOUSE_PALACE_1, 0, 1},
        {GROUP_BUILDING_HOUSE_PALACE_1, 1, 1},
        {GROUP_BUILDING_HOUSE_PALACE_2, 0, 1},
        {GROUP_BUILDING_HOUSE_PALACE_2, 1, 1},
};
...
void building_house_change_to(building *house, int type) {
    ...
    map_building_tiles_add(house->id, house->tile.x(), house->tile.y(), house->size, image_id, TERRAIN_BUILDING);
}
void building_house_change_to_vacant_lot(building *house) {
    ...
    if (house->house_is_merged) {
        ...
        map_building_tiles_add(house->id, house->tile.x(), house->tile.y(), 1, image_id, TERRAIN_BUILDING);
...
static void merge(building *b) {
    ...
    b->tile.set(merge_data.x, merge_data.y);
//    b->tile.x() = merge_data.x;
//    b->tile.y() = merge_data.y;
//    b->tile.grid_offset() = MAP_OFFSET(b->tile.x(), b->tile.y());
    b->house_is_merged = 1;
    map_building_tiles_add(b->id, b->tile.x(), b->tile.y(), 2, image_id, TERRAIN_BUILDING);
}
...
void building_house_merge(building *house) {
    ...
    if (num_house_tiles == 4) {
        merge_data.x = house->tile.x() + expand_delta(0).x;
        merge_data.y = house->tile.y() + expand_delta(0).y;
        merge(house);
    }
}
int building_house_can_expand(building *house, int num_tiles) {
    ...
    for (int dir = 0; dir < MAX_DIR; dir++) {
        ...
        if (ok_tiles == num_tiles) {
            merge_data.x = house->tile.x() + expand_delta(dir).x;
            merge_data.y = house->tile.y() + expand_delta(dir).y;
            return 1;
        }
    }
...
static void create_house_tile(int type, int x, int y, int image_id, int population, const int *inventory) {
    ...
    map_building_tiles_add(house->id, house->tile.x(), house->tile.y(), 1,
                           image_id + (map_random_get(house->tile.grid_offset()) & 1), TERRAIN_BUILDING);
}
...
static void split_size2(building *house, int new_type) {
    ...
    map_building_tiles_add(house->id, house->tile.x(), house->tile.y(), house->size,
                           image_id + (map_random_get(house->tile.grid_offset()) & 1), TERRAIN_BUILDING);

    // the other tiles (new buildings)
    create_house_tile(house->type, house->tile.x() + 1, house->tile.y(), image_id, population_per_tile, inventory_per_tile);
...
void building_house_expand_to_large_insula(building *house) {
    ...
    map_building_tiles_remove(house->id, house->tile.x(), house->tile.y());
    house->tile.set(merge_data.x, merge_data.y);
...
void building_house_devolve_from_large_villa(building *house) {
    ...
    create_house_tile(BUILDING_HOUSE_MEDIUM_INSULA, house->tile.x() + 2, house->tile.y(), image_id, population_per_tile,
                      inventory_per_tile);
    create_house_tile(BUILDING_HOUSE_MEDIUM_INSULA, house->tile.x() + 2, house->tile.y() + 1, image_id, population_per_tile,
                      inventory_per_tile);
    create_house_tile(BUILDING_HOUSE_MEDIUM_INSULA, house->tile.x(), house->tile.y() + 2, image_id, population_per_tile,
                      inventory_per_tile);
    create_house_tile(BUILDING_HOUSE_MEDIUM_INSULA, house->tile.x() + 1, house->tile.y() + 2, image_id, population_per_tile,
                      inventory_per_tile);
    create_house_tile(BUILDING_HOUSE_MEDIUM_INSULA, house->tile.x() + 2, house->tile.y() + 2, image_id, population_per_tile,
                      inventory_per_tile);
}
void building_house_devolve_from_large_palace(building *house) {
    ...
    // the other tiles (new buildings)
    image_id = house_image_group(HOUSE_MEDIUM_INSULA);
    create_house_tile(BUILDING_HOUSE_MEDIUM_INSULA, house->tile.x() + 3, house->tile.y(), image_id, population_per_tile,
                      inventory_per_tile);
    create_house_tile(BUILDING_HOUSE_MEDIUM_INSULA, house->tile.x() + 3, house->tile.y() + 1, image_id, population_per_tile,
                      inventory_per_tile);
    create_house_tile(BUILDING_HOUSE_MEDIUM_INSULA, house->tile.x() + 3, house->tile.y() + 2, image_id, population_per_tile,
                      inventory_per_tile);
    create_house_tile(BUILDING_HOUSE_MEDIUM_INSULA, house->tile.x(), house->tile.y() + 3, image_id, population_per_tile,
                      inventory_per_tile);
    create_house_tile(BUILDING_HOUSE_MEDIUM_INSULA, house->tile.x() + 1, house->tile.y() + 3, image_id, population_per_tile,
                      inventory_per_tile);
    create_house_tile(BUILDING_HOUSE_MEDIUM_INSULA, house->tile.x() + 2, house->tile.y() + 3, image_id, population_per_tile,
                      inventory_per_tile);
    create_house_tile(BUILDING_HOUSE_MEDIUM_INSULA, house->tile.x() + 3, house->tile.y() + 3, image_id, population_per_tile,
                      inventory_per_tile);
}
void building_house_check_for_corruption(building *house) {
    ...
    if (house->tile.grid_offset() != MAP_OFFSET(house->tile.x(), house->tile.y()) || map_building_at(house->tile.grid_offset()) != house->id) {
...
//                    house->tile.grid_offset() = grid_offset; // set house offset to this tile's offset (i.e. lowest x & y; north-west corner)
//                    house->tile.x() = MAP_X(grid_offset); // set house coords (x) to tile's coords (x)
//                    house->tile.y() = MAP_Y(grid_offset); // set house coords (y) to tile's coords (y)
