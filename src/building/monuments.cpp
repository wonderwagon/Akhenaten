...
svector<image_desc, 10> middle_statues_img;
...
void config_load_small_statue_images(archive arch) {
    small_statues_img.clear();
    arch.load_global_array("small_statue_images", [] (archive arch) {
        int pack = arch.read_integer("pack");
        int id = arch.read_integer("id");
        int offset = arch.read_integer("offset");
        small_statues_img.push_back({pack, id, offset});
    });
}
...
int next_statue_variant(int type, int variant) {
    ...
    case BUILDING_MEDIUM_STATUE: size = middle_statues_img.size(); break;
