...
static hufftree8* create_tree8(bitstream* bs) {
    if (read_bit(bs)) {
        ...
        if (!tree) {
            logs::error("SMK: no memory for 8-bit tree");
            return NULL;
        }
        build_tree8_nodes(bs, tree);
        if (read_bit(bs) != 0) {
            logs::error("SMK: 8-bit tree not closed");
            free(tree);
            return NULL;
        }
        return tree;
    } else {
        logs::info("SMK: WARN: no 8-bit tree found");
        return NULL;
    }
}
...
static huffnode16* build_tree16_nodes(bitstream* bs, hufftree16* tree) {
    ...
    if (!node) {
        logs::error("SMK: no memory for 16-bit tree node");
        return NULL;
    }
    if (read_bit(bs)) {
        node->is_leaf = 0;
...
static hufftree16* create_tree16(bitstream* bs, hufftree8* low, hufftree8* high) {
    ...
    if (!tree) {
        logs::error("SMK: no memory for 16-bit tree");
        return NULL;
    }
    tree->low = low;
    tree->high = high;
    ...
    if (read_bit(bs) != 0) {
        logs::error("SMK: 16-bit tree not closed");
        free_tree16(tree);
        return NULL;
    }
    for (int i = 0; i < 3; i++) {
...
static int read_header(smacker s) {
    ...
    if (fread(header, 1, HEADER_SIZE, s->fp) != HEADER_SIZE) {
        logs::error("SMK: unable to read header");
        return 0;
    }
    // check signature
...
static int read_frame_info(smacker s) {
    ...
    if (!s->frame_sizes || !s->frame_offsets || !s->frame_types) {
        logs::error("SMK: no memory for frame info");
        free(s->frame_sizes);
    ...
    if (fread(s->frame_sizes, 1, sizes_length, s->fp) != sizes_length
        ...
        logs::error("SMK: unable to read frame info from file");
        free(s->frame_sizes);
        free(s->frame_offsets);
        free(s->frame_types);
        return 0;
...
static int read_trees_data(smacker s) {
    ...
    if (!trees_data) {
        logs::error("SMK: no memory for tree input data");
        return 0;
    }
    if (fread(trees_data, 1, s->trees_size, s->fp) != s->trees_size) {
        logs::error("SMK: unable to read tree data from file");
        free(trees_data);
        return 0;
    }
...
int allocate_frame_memory(smacker s) {
    ...
    if (!s->frame_data.video) {
        logs::error("SMK: no memory for video frame");
        return 0;
    }
    for (int i = 0; i < MAX_TRACKS; i++) {
        if (s->audio_rate[i] & AUDIO_FLAG_HAS_TRACK) {
            ...
            if (!s->frame_data.audio[i]) {
                logs::error("SMK: no memory for audio track %u", i);
                return 0;
            }
        }
    }
...
smacker smacker_open(FILE* fp) {
    if (!fp) {
        logs::error("SMK: file does not exist");
        return NULL;
    }
...
static int decode_audio_track(smacker s, int track, uint8_t* data, int length) {
    ...
    if (is_stereo != header_is_stereo) {
        logs::error("SMK: stereo flag in frame does not match header");
        return 0;
    }
    if (is_16bit != header_is_16bit) {
        logs::error("SMK: 16-bit flag in frame does not match header");
        return 0;
    }
    ...
    if (!read_audio_frame_trees(bs, trees, num_trees)) {
        logs::error("SMK: unable to read audio huffman trees");
        return 0;
    }
    ...
    while (index < length && color_index < MAX_PALETTE) {
        if (data[index] & 0x80) {
            ...
            if (num_entries + color_index > MAX_PALETTE) {
                logs::error("SMK: invalid palette data");
                return 0;
            }
            ...
            // Literal color
            new_palette[color_index] = (PALETTE_MAP[data[index] & 0x3f] << 16)
...
static uint8_t* read_frame_data(smacker s, int frame_id) {
    if (fseek(s->fp, s->frame_data_offset_in_file + s->frame_offsets[frame_id], SEEK_SET) != 0) {
        logs::error("SMK: unable to seek to frame data %u", frame_id);
        return NULL;
    }
    int frame_size = s->frame_sizes[frame_id];
    uint8_t* frame_data = (uint8_t*)clear_malloc(frame_size);
    if (!frame_data) {
        logs::error("SMK: no memory for frame data %u", frame_id);
        return NULL;
    }
    if (fread(frame_data, 1, frame_size, s->fp) != frame_size) {
        logs::error("SMK: unable to read data for frame %u", frame_id);
        free(frame_data);
        return NULL;
    }
    return frame_data;
