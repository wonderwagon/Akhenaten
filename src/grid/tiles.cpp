    ...
    if (is_updatable_rock(grid_offset)) {
        if (!map_image_at(grid_offset)) {
            if (map_terrain_all_tiles_in_area_are(x, y, 3, TERRAIN_ROCK)
                ...
                map_building_tiles_add(0, map_point(x, y), 3, image_id, TERRAIN_ROCK);
                ...
                map_building_tiles_add(0, map_point(x, y), 2, image_id, TERRAIN_ROCK);
    ...
    if (is_updatable_rock(grid_offset)) {
        if (!map_image_at(grid_offset)) {
            if (map_terrain_all_tiles_in_area_are(x, y, 3, TERRAIN_ORE)
                ...
                map_building_tiles_add(0, map_point(x, y), 3, image_id, TERRAIN_ORE);
                ...
                //                else
                image_id += image_id_from_group(GROUP_TERRAIN_ORE_ROCK);
                map_building_tiles_add(0, map_point(x, y), 2, image_id, TERRAIN_ORE);
    ...
    if (map_terrain_is(grid_offset, TERRAIN_GARDEN)
        ...
        if (!map_image_at(grid_offset)) {
            ...
            if (map_terrain_all_tiles_in_area_are(x, y, 2, TERRAIN_GARDEN)) {
                ...
                map_building_tiles_add(0, map_point(x, y), 2, image_id, TERRAIN_GARDEN);
            } else {
                if (y & 1) {
                    switch (x & 3) {
                    case 0:
...
int map_tiles_set_wall(int x, int y) {
    ...
    foreach_region_tile(tile2i(x - 1, y - 1), tile2i(x + 1, y + 1), set_wall_image);
    return tile_set;
}
...
void map_tiles_update_region_aqueducts(int x_min, int y_min, int x_max, int y_max) {
    foreach_region_tile(tile2i(x_min, y_min), tile2i(x_max, y_max), set_aqueduct_image);
}
...
int map_tiles_set_aqueduct(int x, int y) {
    ...
    foreach_region_tile(tile2i(x - 1, y - 1), tile2i(x + 1, y + 1), set_aqueduct_image);
    return tile_set;
}
...
void map_tiles_update_area_roads(int x, int y, int size) {
    foreach_region_tile(tile2i(x - 1, y - 1), tile2i(x + size - 2, y + size - 2), set_road_image);
}
...
int map_tiles_set_road(int x, int y) {
    ...
    foreach_region_tile(tile2i(x - 1, y - 1), tile2i(x + 1, y + 1), set_road_image);
    return tile_set;
}
