...
void warning_draw(void) {
    ...
    for (int i = 0; i < 5; i++) {
        ...
        if (box_width < 460) {
            ...
            imagedrawnamespace::image_draw(image_id_from_group(GROUP_CONTEXT_ICONS) + 15, center - box_width / 2 + 2, top_offset + 2);
            imagedrawnamespace::image_draw(image_id_from_group(GROUP_CONTEXT_ICONS) + 15, center + box_width / 2 - 30,
                       top_offset + 2);
        }
